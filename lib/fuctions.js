...
exports.smsg = (conn, m, hasParent) => {
  ...
  if(!isNumber(user.clothes)) user.clothes = 'ropa desgastada 👕'
  if(!isNumber(user.sword)) user.sword = 'sin espada'
  if(!isNumber(user.pickaxe)) user.pickaxe = 'sin pico'
  if(!isNumber(user.axe)) user.axe = 'sin hacha'
  if(!isNumber(user.gems)) user.gems = 0
  if(!isNumber(user.gold)) user.gold = 0
  if(!isNumber(user.copper)) user.copper = 0
  if(!isNumber(user.diamonds)) user.diamonds
...
// jid? :v
conn.user.jid = conn.user.id.split(":")[0] + "@s.whatsapp.net"
...
* @param {*} quoted
*/
conn.editMessage = async (jid, text, editedText, quoted) => {
     const { 
          key 
...
* @param {*} quoted
*/
conn.sendDocumentMessage = ( jid, image, fileName, quoted) => {
let MessageDoc = {
document: image,
mimetype: 'text/plain',
fileName: fileName,
}
conn.sendMessage(jid, MessageDoc, { quoted: quoted })
}
	/**
	 * 
	 * @param {*} jid 
