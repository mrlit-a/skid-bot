...
async function videoyt(url, mek, conn, from, user) {
  try {
    ...
    if (videoFormat) {
      ...
      const thumbnailUrl = thumbnail; // Use the thumbnail URL directly

      const caption = `🖥️ ᴛɪᴛᴜʟᴏ: ${title}\n👍 ʟɪᴋᴇs: ${likes}\n👀 ᴠɪsɪᴛᴀs: ${viewCount} \n🔗 ʟɪɴᴋ ${url}`;

      await conn.sendMessage(from, { text: caption, contextInfo: { externalAdReply: { title: `${title}`, body: "epica colaboracion con gata spam 💀", thumbnailUrl: thumbnailUrl, sourceUrl: "wa.me/+528442114446", mediaType: 1, showAdAttribution: false, renderLargerThumbnail: true }}})
