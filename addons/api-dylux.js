...
function formatViews(views) {
  if (views >= 1000000000) {
    return (views / 1000000000).toFixed(1) + 'B'; // Mil millones
  } else if (views >= 1000000) {
    return (views / 1000000).toFixed(1) + 'M'; // Millones
  } else if (views >= 1000) {
    return (views / 1000).toFixed(1) + 'K'; // Miles
  } else {
    return views.toString(); // Números inferiores a 1000
  }
}
...
async function mediafireDl(url) {
  ...
  const filesizeB = (0, parseFileSize)(filesize);
  const result = {
    url: Url || url2,
    url2,
    filename,
...
async function ytmp4(url) {
  ...
  return {
    ...
    views: formatViews(viewCount), // Formatear el número de vistas
    publish: publishDate,
